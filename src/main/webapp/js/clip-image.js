...
window.onload = function() {
	...
	preview.style.left = 650 + "px";
	preview.style.top = 100 + "px";
...
//		}
	...
	mainBox.addEventListener('touchstart',touch, false);
	mainBox.addEventListener('touchmove',touch, false);
	mainBox.addEventListener('touchend',touch, false);
};
...
function moveMainBoxOnMobile(downX, downY, upX, upY) {
	...
	var cropimgElemHeight = cropimgElem.clientHeight;
	var cropimgElemPosition = getPosition(cropimgElem);
	var minX = 0;// 待裁剪的图片最小x坐标
	var maxX = 305;// 待裁剪的图片最大x坐标
	var minY = 0// 待裁剪的图片最小y坐标
	var maxY = 160;// 待裁剪的图片最大y坐标

	var Y = mainBox.offsetTop+upY-downY;
	var X = mainBox.offsetLeft+upX-downX;

	console.log(minX+"-"+minY+"-"+maxX+"-"+maxY +"-"+mainBox+"-"+Y);
	console.log("X="+X+"  Y="+Y);
	if(X<maxX && X>minX && Y<maxY && Y>minY){
		console.log("set X="+X+" and Y="+Y)
	...
	var c = document.getElementById("cropimg2");
	c.style.clip = "rect(" + top + "px, " + right + "px, " + bottom + "px, " + left + "px)";
	var previewImg = document.getElementById("cropimg3");
	previewImg.style.top = -top + "px";
	previewImg.style.left = -left + "px";
	previewImg.style.clip = "rect(" + top + "px, " + right + "px, " + bottom + "px, " + left + "px)";
...
function touch (event){
	...
	switch(event.type){
		case "touchstart":
			...
			downY = event.changedTouches[0].clientY;
			break;
		case "touchend":
			upX = event.changedTouches[0].clientX;
			upY = event.changedTouches[0].clientY;
			moveMainBoxOnMobile(downX, downY, upX, upY);
			break;
		case "touchmove":
			event.preventDefault();
			downX=upX;
